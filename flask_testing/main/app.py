...
def create_app():
    ...
    def get_client_by_id(client_id: int):
        client: Client = db.session.query(Client).get(client_id)  # type: ignore # noqa
        return jsonify(client.to_json()), 200

    @app.route("/parkings/<int:parking_id>", methods=["GET"])
    def get_parking_by_id(parking_id: int):
        parking: Parking = db.session.query(Parking).get(parking_id)  # type: ignore # noqa
    ...
    def delete_client_parking():
        ...
        client_parking: Client_Parking = db.session.query(Client_Parking).get(
            client_id
        )  # noqa
